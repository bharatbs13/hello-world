...
`v0.2.1` foundation  
`v0.3+` expanded multi-connector usage
...
Example:
N connectors = 10
source_target combinations = 10 × 10 = 100
...
solution defines topology shape
connector pool defines available connector types
planner selects concrete combinations
selected combinations become D profiles
 
⸻
 
4. Terminology
...
AC1
D profiles must be generated per run, not globally hardcoded.
AC2
Reports must include the D mapping used for that run.
AC3
...
strategies.
AC4
Planner must support cap:
max_depth_profiles
AC5
Random strategy must support deterministic replay using:
random_seed
AC6
D profiles must be available to:
feature
regression
load
performance
parallelism
but only used when applicable.
AC7
Solution-specific workload/test code must receive resolved D context.
AC8
Automation must not hardcode connector combinations in test runner logic.
 
⸻
 
16. Final Decision
